...
export async function fetchCoverArt(
	...
	artistData: MusicBrainzArtist | null;
} | null> {
	try {
		...
		const searchResponse = await fetch(searchUrl);
		const searchData: MusicBrainzRecordingResponse =
		...
		for (const recording of searchData.recordings) {
			...
			const studioAlbums = recording.releases.filter((release) => {
				...
				const primaryType =
					release["release-group"]?.["primary-type"]?.toLowerCase();
				const secondaryTypes =
					release["release-group"]?.["secondary-types"]?.map((t) =>
						t.toLowerCase(),
					) || [];
