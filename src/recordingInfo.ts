import type { MusicBrainzArtist, MusicBrainzRecording } from "./musicbrainz.ts";
import { formatArtistCredit, formatDate, formatDuration } from "./utils.ts";
...
export function displayRecordingInfo(
	...
	if (artistData) {
		...
		if (artistData.tags && artistData.tags.length > 0) {
			...
			const artistTagsHtml = artistData.tags
				.map((tag) => `<span class="recording-tag">${tag.name}</span>`)
				.join("");
...
export function hideRecordingInfo(): void {
	...
	if (!recordingInfoContent) return;
	recordingInfoContent.innerHTML =
		'<div class="loading">No recording information available</div>';
}
