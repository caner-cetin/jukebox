...
const server = Bun.serve({
	...
	routes: {
		...
		"/public/*": async (req) =>
			new Response(
				await Bun.file(`./public/${req.url.split("/").reverse()[0]}`).bytes(),
			),
		"/dist/*": async (req) =>
			new Response(
				await Bun.file(`./dist/${req.url.split("/").reverse()[0]}`).bytes(),
			),
		"/api/yuri": async (req) => {
		...
		"/api/station/:id/queue": async (req) => {
			const response = await fetch(
				`https://radio.cansu.dev/api/station/${req.params.id}/queue`,
				{ headers: { "X-API-Key": process.env.AZURACAST_API_KEY || "" } },
			);
